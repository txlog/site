...
Txlog Server dashboard to quickly locate specific assets and identify stale or
inactive systems.
...
filtering. For example, to find assets that need a reboot, type `restart:true`
in the search field.
...
> restart:true` will find production servers that require a restart.
